...
describe("DepositService", () => {
  ...
  it("should throw an error if the deposit amount exceeds 25% of total job cost", async () => {
    ...
    await expect(
      depositService.deposit(clientId, depositAmount),
    ).rejects.toThrow(
  ...
  it("should handle errors and rollback the transaction", async () => {
    mockJobRepo.getTotalJobsCost.mockResolvedValue({ total: 100 }); // Mock total job cost
    mockProfileRepo.findByPk.mockRejectedValue(
      new Error("Error retrieving client data"),
    ); // Simulate an error
    ...
    await expect(
      depositService.deposit(clientId, depositAmount),
    ).rejects.toThrow("Error retrieving client data");
  });
});
