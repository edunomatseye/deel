...
describe('ContractsService', () => {
  ...
  describe('getContractList', () => {
    ...
    it('should handle errors when retrieving contracts', async () => {
      ...
      ContractsRepository.prototype.listNonTerminatedContracts.mockRejectedValue(expectedError);

      // Act
      const contractsService = new ContractsService();

      // Assert and check for rejection
      await expect(contractsService.getContractList(profileId)).rejects.toThrow(expectedError);
    });
  });
});
